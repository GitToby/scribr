...
pub fn get_default_init_files() -> HashMap<String, File> {
    HashMap::from([(
        SCRIBR_CONFIG_FILE_NAME.to_string(),
        File {
            content: serde_yaml::to_string(&Settings::default())
                .expect("Default settings are incorrect."),
        },
    )])
}
...
pub fn get_settings(settings_file: Option<PathBuf>) -> Settings {
    ...
    if let Some(remote) = &settings.remote {
        if remote.token == None || remote.gist_id == None {
...
mod tests {
    ...
    fn test_get_default_settings_none_asked() {
        let s = get_settings(None);
        assert_eq!(s, Settings::default())
    }
    #[test]
    fn test_get_default_settings_path_not_exists() {
        let s = get_settings(Some(PathBuf::from("this/path/probably/does/not/exists")));
        assert_eq!(s, Settings::default())
    }
    ...
    fn test_get_default_settings_path_empty_remote() {
        ...
        let actual = get_settings(Some(resources_dir));
    ...
    fn test_get_default_settings_path_exists_remote() {
        ...
        let expected = Settings {
            ...
            remote: Some(RemoteSettings {
                ...
                token: Some("tests-token".to_string()),
            }),
        };
        assert_eq!(actual, expected)
    }
